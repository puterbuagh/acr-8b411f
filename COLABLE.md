...
- **Auth**: Supabase email/password + OAuth (Google) with SSR middleware protecting dashboard/settings/history, force-dynamic export on auth pages for createClient compatibility, polished poker-themed auth UI with felt-inspired gradients, refined card shadows, editorial typography, smooth hover/focus transitions, **trimmed email input validation** and explicit error logging in LoginForm
...
- **Auth**: AuthCard scaffold with felt-inspired gradient backgrounds, animated gradient orbs, decorative poker chip elements, refined card shadows; async server LoginForm/SignupForm with force-dynamic export and default exports, createClient imports, **trimmed email validation**, **explicit error logging with session structure validation**, smooth hover/focus transitions, emerald accent colors; OAuthButtons with Google logo, polished styling; AuthDivider; cohesive poker-themed visual language across all auth components
